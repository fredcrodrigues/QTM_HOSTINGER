$(document).on('change', '#filtro', function() {
    ...
    $("#filter").on('click', function(event) {
        ...
        event.preventDefault();
            var dados = new FormData($("#coworkings")[0]);
            $.ajax({
                url: base_url + '/busca-coworkings',
                type: 'post',
                headers: {
...
function initialize(filter) {
    ...
    filter.forEach(function(item){
        locations.push(["<p style='margin: 0; padding:0; padding: 2px;'>Nome do Local: <b>"+item.nome+"</b></p><p style='margin: 0; padding:0; padding: 2px;'></p><br><a href='/agendamento/coworking/"+item.slug+"' target='_blank' class='btn btn-transparent-deep-pink btn-rounded no-margin-right'>agendar</a><br>"
